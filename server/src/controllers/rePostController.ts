...
export const createRePost = async (req: Request, res: Response) => {
  ...
  if (!postId || !userId)
    return res.status(400).json({
      status: "FAILED",
      data: { error: "Post Id and user Id required" },
    });
  ...
  res.status(200).json({ status: "OK", data: rePost });
};
...
export const likeRePost = async (req: Request, res: Response) => {
  ...
  if (!rePost)
    return res.status(404).json({
      status: "FAILED",
      data: { error: "No repost with matching Id" },
    });
...
export const updateRePost = async (req: Request, res: Response) => {
  ...
  res.status(200).json({ status: "OK", data: updatedRePost });
};
...
export const deleteRePost = async (req: Request, res: Response) => {
  ...
  res.status(204).json({ status: "OK", data: deletedRePost });
};
